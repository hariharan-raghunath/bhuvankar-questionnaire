...
export async function POST(request) {
  try {
    ...
    // Check if customer exists (by encrypted PAN)
    const existingCustomer = await prisma.clientSubmission.findUnique({
      where: { panNumber: encryptedPan.encrypted }
    });
    ...
    if (existingCustomer) {
      ...
      submission = await prisma.clientSubmission.update({
        where: { panNumber: encryptedPan.encrypted },
        data: {
          ...submissionData,
          version: existingCustomer.version + 1  // Increment version
        }
      });
      isUpdate = true;
    } else {
    ...
    return NextResponse.json({ 
      ...
      clientNumber: submission.id,  // Human-readable ID
      isUpdate: isUpdate,
      message: isUpdate ? 'Details updated successfully' : 'New submission created'
    });
